...
<!-- 1. Render only your links/menu, measure its height -->
<ProjectPageMenuMobile on:linksheight={onLinkHeight} {project} />

<!-- 2. Render the description here, behind the scrollable layer -->
...
<div
    ...
    position: absolute;
    top: {linkHeight}px;
    height: calc(100vh - {linkHeight}px);
    overflow-y: auto;
    z-index: 20;
  "
>
				...
				>
					{#each row.images as item}
						{#if item.type === 'image' && item.image}
							<img src={item.image.asset.url} alt={project.title} style={`width: ${item.width}`} />
